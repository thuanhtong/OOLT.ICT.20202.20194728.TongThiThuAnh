...
public class Cart {
	...
	public Media getALuckyItem() { //->getFreeItem();
		ArrayList<Integer> list= new ArrayList<Integer>();
		for(int i=0; i<itemsOrdered.size(); i++) {
			if(itemsOrdered.get(i).getCost() < totalCost()/itemsOrdered.size()) { //get only items which have lower price than totalCost/itemsOrdered.size()
				list.add(i);
			}
		}
		int rand = (int)(Math.random()*list.size());
		Media tmp=itemsOrdered.get(list.get(rand));
		removeMedia(itemsOrdered.get(list.get(rand)));
	...
	public void getFreeItem()  {
		...
		if(itemsOrdered.size()>=5 && totalCost()>= 100.0f) {
			if(flag==0) {
			...
			JOptionPane.showMessageDialog(null,"Can't get a lucky item.\nTo get lucky item The number of items must be bigger than 5 and Total cost must be bigger than 100$!", "Get Lucky Item", JOptionPane.ERROR_MESSAGE);
		}
	}
